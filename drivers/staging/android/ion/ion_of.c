				    ...
				    struct ion_of_heap *compatible)
{
	...
	pr_debug("%s: id %d type %d name %s align %llu\n", __func__,
...
static int rmem_ion_device_init(struct reserved_mem *rmem, struct device *dev)
{
	...
	pr_debug("%s: heap %s base %pa size %luMiB dev %p\n", __func__,
		 heap->name, &rmem->base, size, dev);
	return 0;
}
...
static int __init rmem_ion_setup(struct reserved_mem *rmem)
{
	...
	pr_info("Ion memory setup at %pa size %lu MiB\n", &rmem->base, size);
