...
#define LSM9DS0_ACCEL_ODR_MASK				0xf0
...
#define LSM9DS0_MAGN_ODR_MASK				0x1c
...
#define LSM9DS0_GYRO_ODR_MASK			0xc0
...
static const struct sensor_odr_avl lsm9ds0_accel_odr_avl[] = {
	{ 3, LSM9DS0_ACCEL_ODR_3_125HZ_VAL },
	{ 6, LSM9DS0_ACCEL_ODR_6_25HZ_VAL },
	{ 12, LSM9DS0_ACCEL_ODR_12_5HZ_VAL },
	{ 25, LSM9DS0_ACCEL_ODR_25HZ_VAL },
	{ 50, LSM9DS0_ACCEL_ODR_50HZ_VAL },
	{ 100, LSM9DS0_ACCEL_ODR_100HZ_VAL },
	{ 200, LSM9DS0_ACCEL_ODR_200HZ_VAL },
	{ 400, LSM9DS0_ACCEL_ODR_400HZ_VAL },
	{ 800, LSM9DS0_ACCEL_ODR_800HZ_VAL },
	{ 1600, LSM9DS0_ACCEL_ODR_1600HZ_VAL },
};

static const struct sensor_odr_avl lsm9ds0_magn_odr_avl[] = {
		{ 3, LSM9DS0_MAGN_ODR_3_125HZ_VAL },
		{ 6, LSM9DS0_MAGN_ODR_6_25HZ_VAL },
		{ 12, LSM9DS0_MAGN_ODR_12_5HZ_VAL },
		{ 25, LSM9DS0_MAGN_ODR_25HZ_VAL },
		{ 50, LSM9DS0_MAGN_ODR_50HZ_VAL },
		{ 100, LSM9DS0_MAGN_ODR_100HZ_VAL },
};

static const struct sensor_odr_avl lsm9ds0_gyro_odr_avl[] = {
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (strcmp(attr->attr.name, "in_anglvel_scale_available") == 0) {
...
static ssize_t lsm9ds0_show_freq_avail(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	size_t len = 0;
	int n;
	const struct sensor_odr_avl (*avl)[];

	if (strcmp(attr->attr.name, "in_anglvel_sampling_frequency_available") == 0) {
		avl = &lsm9ds0_gyro_odr_avl;
		n = ARRAY_SIZE(lsm9ds0_gyro_odr_avl);
	} else if (strcmp(attr->attr.name, "in_accel_sampling_frequency_available") == 0) {
		avl = &lsm9ds0_accel_odr_avl;
		n = ARRAY_SIZE(lsm9ds0_accel_odr_avl);
	} else if (strcmp(attr->attr.name, "in_magn_sampling_frequency_available") == 0) {
		avl = &lsm9ds0_magn_odr_avl;
		n = ARRAY_SIZE(lsm9ds0_magn_odr_avl);
	} else {
		return -EINVAL;
	}
...
static IIO_DEVICE_ATTR(in_accel_sampling_frequency_available, S_IRUGO,
	lsm9ds0_show_freq_avail, NULL, 0);
static IIO_DEVICE_ATTR(in_magn_sampling_frequency_available, S_IRUGO,
	lsm9ds0_show_freq_avail, NULL, 0);
static IIO_DEVICE_ATTR(in_anglvel_sampling_frequency_available, S_IRUGO,
	lsm9ds0_show_freq_avail, NULL, 0);

static struct attribute *lsm9ds0_gyro_attributes[] = {
	&iio_dev_attr_in_anglvel_scale_available.dev_attr.attr,
	&iio_dev_attr_in_anglvel_sampling_frequency_available.dev_attr.attr,
	NULL
};
			...
			int *val, int *val2, long mask)
{
	...
	const struct sensor_odr_avl (*odr_avl)[];
	int err = 0, n = 0, i = 0, ret = -1;
	s16 x = 0, y = 0, z = 0;
	int scale = 0;
	...
	case IIO_CHAN_INFO_SAMP_FREQ:
		switch (channel->type) {
		case IIO_ANGL_VEL:
			odr_avl = &lsm9ds0_gyro_odr_avl;
			n = ARRAY_SIZE(lsm9ds0_gyro_odr_avl);
			ret = data->gyro_sfreq;
			break;
		case IIO_ACCEL:
			odr_avl = &lsm9ds0_accel_odr_avl;
			n = ARRAY_SIZE(lsm9ds0_accel_odr_avl);
			ret = data->accel_sfreq;
			break;
		case IIO_MAGN:
			odr_avl = &lsm9ds0_magn_odr_avl;
			n = ARRAY_SIZE(lsm9ds0_magn_odr_avl);
			ret = data->magn_sfreq;
			break;
		default:
			...
			int val, int val2, long mask)
{
	...
	const struct sensor_fs_avl (*fs_avl)[];
	const struct sensor_odr_avl (*odr_avl)[];
	int n, i, ret;
	...
	case IIO_CHAN_INFO_SCALE:
		...
		ret = -EINVAL;
		for (i = 0; i < n; i++) {
			if ((*fs_avl)[i].gain == val2) {
				ret = 0;
				new_value = (*fs_avl)[i].value;
				break;
			}
		}
		if (ret < 0)
	...
	case IIO_CHAN_INFO_SAMP_FREQ:
		...
		dev_info(&client->dev, "Sampling rate = %d\n", val);
		switch (channel->type) {
		case IIO_ANGL_VEL:
			odr_avl = &lsm9ds0_gyro_odr_avl;
			n = ARRAY_SIZE(lsm9ds0_gyro_odr_avl);
			reg_address = LSM9DS0_CTRL_REG1_G_REG;
			reg_mask = LSM9DS0_GYRO_ODR_MASK;
			in_data = &(data->gyro_sfreq);
			break;
		case IIO_ACCEL:
			odr_avl = &lsm9ds0_accel_odr_avl;
			n = ARRAY_SIZE(lsm9ds0_accel_odr_avl);
			reg_address = LSM9DS0_CTRL_REG1_XM_REG;
			reg_mask = LSM9DS0_ACCEL_ODR_MASK;
			in_data = &(data->accel_sfreq);
			break;
		case IIO_MAGN:
			odr_avl = &lsm9ds0_magn_odr_avl;
			n = ARRAY_SIZE(lsm9ds0_magn_odr_avl);
			reg_address = LSM9DS0_CTRL_REG5_XM_REG;
			reg_mask = LSM9DS0_MAGN_ODR_MASK;
			in_data = &(data->magn_sfreq);
			break;
		default:
			ret = -EINVAL;
			goto done;
...
static irqreturn_t lsm9ds0_trigger_top(int irq, void *p)
{
...
static irqreturn_t lsm9ds0_trigger_h(int irq, void *p)
{
	...
	s16 *buf_data;
	int i, j;
	s16 x1 = 0, y1 = 0, z1 = 0, x2 = 0, y2 = 0, z2 = 0;
	int err;

	buf_data = kzalloc(indio_dev->scan_bytes, GFP_KERNEL);
	if (!buf_data)
		goto done;
	...
	if (!bitmap_empty(indio_dev->active_scan_mask, indio_dev->masklength)) {
		...
		if (data->sensor_type == GYRO) {
			...
			if ( test_bit(SCAN_INDEX_ACCEL_X, indio_dev->active_scan_mask) ||
					test_bit(SCAN_INDEX_ACCEL_Y, indio_dev->active_scan_mask) ||
					test_bit(SCAN_INDEX_ACCEL_Z, indio_dev->active_scan_mask)) {
				err = lsm9ds0_read_measurements(data->client,
						LSM9DS0_OUT_X_L_A_REG, &x1, &y1, &z1);
				if (err < 0)
					goto free_buf;
			}
			if ( test_bit(SCAN_INDEX_MAGN_X, indio_dev->active_scan_mask) ||
					test_bit(SCAN_INDEX_MAGN_Y, indio_dev->active_scan_mask) ||
					test_bit(SCAN_INDEX_MAGN_Z, indio_dev->active_scan_mask)) {
				err = lsm9ds0_read_measurements(data->client,
	...
	iio_push_to_buffers_with_timestamp(indio_dev, buf_data, pf->timestamp);
...
static int lsm9ds0_accel_magn_init(struct i2c_client *client)
{
	...
	data->accel_sfreq = LSM9DS0_ACCEL_ODR_100HZ_VAL;
	data->magn_sfreq = LSM9DS0_MAGN_ODR_50HZ_VAL;
